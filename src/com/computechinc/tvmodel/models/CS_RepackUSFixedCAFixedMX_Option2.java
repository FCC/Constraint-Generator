...
public class CS_RepackUSFixedCAFixedMX_Option2 extends AbstractConstrsGenr {
	//========================================================================//
	// Class level
	//========================================================================//
	...
	// constructors
	public CS_RepackUSFixedCAFixedMX_Option2(Profile profile) throws Exception {
		super(profile);
		//
		init();
	}
	...
	protected void determineConsideredStations() throws Exception {
		...
		//
		{
			...
			for (Station station : stationId2ConsideredStation.values()) {
				...
				if (origBand.equalsIgnoreCase(Channel.UHF)) {
					super.consideredUHFStations.add(station);
				}
				else if (origBand.equalsIgnoreCase(Channel.UpperVHF)) {
					super.consideredUpperVHFStations.add(station);
				}
				else if (origBand.equalsIgnoreCase(Channel.LowerVHF)) {
					super.consideredLowerVHFStations.add(station);
				}
				else {
		...
		//
		super.stationId2ProtectedStation.putAll(CanadaStation.getStationId2DTCanadaStation());
		//super.stationId2ProtectedStation.putAll(CanadaStation.getStationId2LDCanadaStation());
		super.stationId2ProtectedStation.putAll(CanadaStation.getStationId2TACanadaStation());
		super.stationId2ProtectedStation.putAll(CanadaStation.getStationId2TVCanadaStation());
		//super.stationId2ProtectedStation.putAll(CanadaStation.getStationId2TXCanadaStation());
		//
		...
		try {
			...
			for (HashMap<String, Object> record : records) {
				...
				if (stationId2ConsideredStation.containsKey(studiedFacilityId) && stationId2ConsideredStation.containsKey(interferingFacilityId)) {
					...
					// studiedStation is US and interferingStation is Canada
					// Only process record where the studied channel equals the protected station original channel 
					if (interferingStation.getCountry().equalsIgnoreCase("CA") && interferingOrigChannel.equals(interferingChannel)) {
						...
						//
						if (studiedChannelNum.longValue() == interferingChannelNum.longValue()) {
							if (interferedServedPopPct.doubleValue() > ignoredInterfPctUB) {
								// CoChannel short-spaced stations
								if (studiedOrigChannel.equals(interferingOrigChannel)) {
									ConstrsGenrDriver.displayErr("Error: Inconsistent CoChannel short-spaced pair [" + studiedStation + "," + interferingStation + "," + interferedServedPopPct + "%] for Option 2");
									dataIntegrityStatus = false;
									continue;
								}
								else {
									studiedStation.removeChannel(studiedChannel);
								}
							}
						}
						// Look up
						else if (studiedChannelNum.longValue() + 1 == interferingChannelNum.longValue()) {
							if (interferedServedPopPct.doubleValue() > ignoredInterfPctUB) {
								Channel interferingOrigChannelBelow = interferingOrigChannel.getAdjChannelBelow();
								// CoChannel short-spaced stations
								if (studiedOrigChannel.equals(interferingOrigChannel)) {
									ConstrsGenrDriver.displayErr("Error: Inconsistent CoChannel==>AdjChannel above short-spaced pair [" + studiedStation + "," + interferingStation + "," + interferedServedPopPct + "%] for Option 2");
									dataIntegrityStatus = false;
									continue;
								}
								// AdjChannel short-spaced stations
								else if (interferingOrigChannelBelow != null && studiedOrigChannel.equals(interferingOrigChannelBelow)) {
									ConstrsGenrDriver.displayErr("Error: Inconsistent AdjChannel above short-spaced pair [" + studiedStation + "," + interferingStation + "," + interferedServedPopPct + "%] for Option 2");
									dataIntegrityStatus = false;
									continue;
								}
								else if (interferingOrigChannelBelow != null) {
									studiedStation.removeChannel(studiedChannel);
								}
								else {
									// skip
						...
						// Look down
							...
							if (interferedServedPopPct.doubleValue() > ignoredInterfPctUB) {
								Channel interferingOrigChannelAbove = interferingOrigChannel.getAdjChannelAbove(); 
								// CoChannel short-spaced stations
								if (studiedOrigChannel.equals(interferingOrigChannel)) {
									ConstrsGenrDriver.displayErr("Error: Inconsistent CoChannel==>AdjChannel below short-spaced pair [" + studiedStation + "," + interferingStation + "," + interferedServedPopPct + "%] for Option 2");
									dataIntegrityStatus = false;
									continue;
								}
								// AdjChannel short-spaced stations
								else if (interferingOrigChannelAbove != null && studiedOrigChannel.equals(interferingOrigChannelAbove)) {
									ConstrsGenrDriver.displayErr("Error: Inconsistent AdjChannel below short-spaced pair [" + studiedStation + "," + interferingStation + "," + interferedServedPopPct + "%] for Option 2");
									dataIntegrityStatus = false;
									continue;
								}
								else if (interferingOrigChannelAbove != null) {
									studiedStation.removeChannel(studiedChannel);
								}
								else {
									// skip
							...
							ConstrsGenrDriver.displayErr("Error: Inconsistent consecutive channels on stations pair [" + studiedStation + "," + interferingStation + "," + interferedServedPopPct + "%] for Option 2");
							dataIntegrityStatus = false;
							continue;
						}
					}
					...
					// studiedStation is Canada and interferingStation is US
					// Visit only record where the studied channel equals the protected station original channel 
					if (studiedStation.getCountry().equalsIgnoreCase("CA") && studiedOrigChannel.equals(studiedChannel)) {
						...
						if (studiedChannelNum.longValue() == interferingChannelNum.longValue()) {
							if (interferedServedPopPct.doubleValue() > ignoredInterfPctUB) {
								...
								if (studiedOrigChannel.equals(interferingOrigChannel)) {
									...
									interferingStation.removeChannel(interferingChannel);
								}
							}
						}
						// Look up
							...
							if (interferedServedPopPct.doubleValue() > ignoredInterfPctUB) {
								Channel studiedOrigChannelAbove = studiedOrigChannel.getAdjChannelAbove();
								// CoChannel short-spaced stations
								if (studiedOrigChannel.equals(interferingOrigChannel)) {
									ConstrsGenrDriver.displayErr("Error: Inconsistent CoChannel==>AdjChannel above short-spaced pair [" + studiedStation + "," + interferingStation + "," + interferedServedPopPct + "%] for Option 2");
									dataIntegrityStatus = false;
								...
								// AdjChannel short-spaced stations
								else if (studiedOrigChannelAbove != null && studiedOrigChannelAbove.equals(interferingOrigChannel)) {
									ConstrsGenrDriver.displayErr("Error: Inconsistent AdjChannel above short-spaced pair [" + studiedStation + "," + interferingStation + "," + interferedServedPopPct + "%] for Option 2");
									dataIntegrityStatus = false;
									continue;
								}
								else if (studiedOrigChannelAbove != null) {
									interferingStation.removeChannel(interferingChannel);
								}
								else {
									// skip
						...
						// Look down
							...
							if (interferedServedPopPct.doubleValue() > ignoredInterfPctUB) {
								Channel studiedOrigChannelBelow = studiedOrigChannel.getAdjChannelBelow(); 
								// CoChannel short-spaced stations
								if (studiedOrigChannel.equals(interferingOrigChannel)) {
									ConstrsGenrDriver.displayErr("Error: Inconsistent CoChannel==>AdjChannel below short-spaced pair [" + studiedStation + "," + interferingStation + "," + interferedServedPopPct + "%] for Option 2");
									dataIntegrityStatus = false;
								...
								// AdjChannel short-spaced stations
								else if (studiedOrigChannelBelow != null && studiedOrigChannelBelow.equals(interferingOrigChannel)) {
									ConstrsGenrDriver.displayErr("Error: Inconsistent AdjChannel below short-spaced pair [" + studiedStation + "," + interferingStation + "," + interferedServedPopPct + "%] for Option 2");
									dataIntegrityStatus = false;
									continue;
								}
								else if (studiedOrigChannelBelow != null) {
									interferingStation.removeChannel(interferingChannel);
								}
								else {
									// skip
							...
							ConstrsGenrDriver.displayErr("Error: Inconsistent studied and interfering channels on stations pair [" + studiedStation + "," + interferingStation + "," + interferedServedPopPct + "%] for Option 2");
							dataIntegrityStatus = false;
							continue;
						}
					}
